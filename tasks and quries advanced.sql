...
SELECT 
		...
		bk.book_title,
		ist.issued_date,
		CURRENT_DATE - ist.issued_date AS due_day-- DUE DAY
FROM 
		issued_status AS ist
JOIN 
		members AS m 
...
LEFT JOIN 
	...
	ON rs.issued_id=ist.issued_id
WHERE 
		return_date IS NULL AND CURRENT_DATE - ist.issued_date > 30
ORDER BY 1;
...
SELECT  e.emp_name,
		br.*,
		COUNT(ist.issued_id) AS no_book_issue
	FROM employees as e
JOIN 
		issued_status as ist
	ON e.emp_id = ist.issued_emp_id
...
status "damaged" in the books table. Display the member name, book title, and the number of 
...
SELECT m.member_name,
		bk.book_title,
		COUNT(rs.book_quality) AS no_damaged_book
	FROM issued_status AS ist
JOIN members as m
...
Description: 
Write a CTAS query to create a new table that lists each member and the 
books they have issued but not returned within 30 days.
...
CREATE TABLE overdue_fines_summary 
AS
SELECT  m.member_id,
		m.member_name,
		COUNT(*) AS total_issued_book,
		SUM (CASE 
				WHEN CURRENT_DATE - ist.issued_date > 30 and rs.return_date IS NULL
